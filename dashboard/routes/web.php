...
// a dashboard account is signed in (also linked from the staff sidebar nav). When
...
// dashboard shell. Keyed by username directly (works for offline players wherever the
// mod's own endpoint supports it). Gated end-to-end (reads included, not just writes)
// since this prefix isn't nested under /dashboard's blanket protection anymore — see
// PlayerProfileController's class docblock for the rest of the rationale. Three
// read-only siblings (lookup/balance/permission-info) deliberately stayed at their
// original dashboard.players.profile.* routes below — Economy.tsx and Players.tsx's
// own preview features call those too, for any authenticated user, not just admins.
Route::middleware(['auth', 'verified', 'can:players.profile.manage'])
...
// Shared lookups for the panel above (permission-group dropdown, jail-name dropdown) —
// no {username}, so kept outside the prefix group but under the same gate.
Route::middleware(['auth', 'verified', 'can:players.profile.manage'])->group(function () {
    Route::get('/lookup-groups', [PlayerProfileController::class, 'groups'])->name('lookup.manage.groups');
    Route::get('/lookup-jails', [PlayerProfileController::class, 'jails'])->name('lookup.manage.jails');
});
...
Route::middleware(['auth', 'verified', 'account.linked'])->prefix('dashboard')->group(function () {
    ...
    Route::name('dashboard.')->group(function () {
        ...
        // Read-only, still shared with Economy.tsx's and Players.tsx's own player-lookup
        // previews (any authenticated user, not gated to admins) — the rest of what used
        // to live here (the full management page + its ~40 other routes) moved to
        // /lookup/{username}, see above.
